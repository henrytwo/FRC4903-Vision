...
class AutoTarget:
    def __init__(self, headless, camID):
        ...
        os.system('v4l2-ctl -d %s -c white_balance_temperature_auto=0' % camID)
        os.system('v4l2-ctl -d %s -c white_balance_temperature=0' % camID)
        os.system('v4l2-ctl -d %s -c saturation=100' % camID)
        os.system('v4l2-ctl -d %s -c contrast=100' % camID)
        os.system('v4l2-ctl -d %s -c exposure_auto=1' % camID)
        os.system('v4l2-ctl -d %s -c exposure_absolute=0' % camID)
        os.system('v4l2-ctl -d %s -c brightness=50' % camID)

        print('Cam ID %s succeeded' % camID)
    ...
    def run(self):
        while True:
            ...
            self.frame = cv2.imread('target.jpg', 1)
            ...
            for l in range(len(left)):
                ...
                if not self.headless:
                    ...
                    dist_to_centre = -1 * (rect[0] + rect[2] // 2 - self.WIDTH // 2)
                    ...
                    cv2.line(self.frame, (rect[0] + rect[2] // 2, 0), (rect[0] + rect[2] // 2, self.HEIGHT),
                             (0, 255, 0), 1)

                    if abs(dist_to_centre) < abs(min_deviation):
                        min_deviation = dist_to_centre

                        closest_line_x = rect[0] + rect[2] // 2
            ...
            if min_deviation != 999999:
                ...
                cv2.line(self.frame, (closest_line_x, 0), (closest_line_x, self.HEIGHT),
                         (0, 255, 255), 1)
                ...
                if not self.headless:
                    ...
                    cv2.putText(self.frame,
                                'Deviation: %i | (This means you %s)' % (
                                    min_deviation, deviation_msg),
